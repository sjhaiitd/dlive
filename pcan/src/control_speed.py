					...
					print "\nSpeed of Car : ", int(DATA7,16)
				        ...
				        setpoint= Float64(setpoint1)
                                        ...
                                        if(cmn_first==False):
                                           ...
                                           car_sp.append([float(carspeed1),float(setpoint1),prev_time-first_time])
					   ...
					   pub1.publish(carspeed)
                                          ...
                                          # if((float(carspeed1)/float(setpoint1)*100>90) and first==True):
                                                ...
                                                # first=False
                                           if(bprev-bpprev>0 and carspeed1-bprev<0 and firstocl==True and bprev>setpoint):
                                                 print "max",bprev
		                                 ...
		                                 firstocl=False
                                           ...
                                           if(count>100):
    	                                          ss_error=e
                                           if(bprev-bpprev<0.001 and carspeed1-bprev<0.001 and firstst==True):
					   ...
					   print "setpoint",setpoint
                                           ...
                                           para_list=[rise_time,settling_time,overshoot,ss_error,float(carspeed1)]
...
def callback1(msg):
    ...
    if msg.key=='q': #### STOPPING PART
       ...
       if(steer>=0):
          ...
          e2OCAN_LAMP.DATA[6] = LAMP_BKLIGHT_B6_OFF
          #e2OCAN_BRAKE.DATA[7] = BRAKE_ZERO
    elif(msg.key=="w" and neutral == False): #start
       cmn_first=False
       stop_code=False
       car_sp=[]
       #print"lll"
    elif(msg.key=="s" and neutral == False):#stop
       setpoint1=0
       e2OCAN_RNDB.DATA[6] = RNDB_DRIVE
       e2OCAN_ACCEL.DATA[7] =ACCEL_ZERO
       e2OCAN_BRAKE.DATA[7] = BRAKE_ZERO
       control=False
    elif(msg.key=="1" and neutral == False):
       e2OCAN_STEER.DATA[7] =STEER_RIGHT
    elif(msg.key=="2" and neutral == False):
       setpoint1=2
    elif(msg.key=="3" and neutral == False):
       setpoint1=3
    elif(msg.key=="4" and neutral == False):
       setpoint1=4
    elif(msg.key=="5" and neutral == False):
       setpoint1=5
    elif(msg.key=="6" and neutral == False):
       setpoint1=6
    elif(msg.key=="7" and neutral == False):
       setpoint1=7
    elif(msg.key=="8" and neutral == False):
       setpoint1=8
    elif(msg.key=="9" and neutral == False):
       setpoint1=9
    elif(msg.key=="j" and neutral == False):#log 
       print_paras=True
    elif(msg.key=="k" and neutral == False):#resetting
       reset_values=True
    elif(msg.key=="i") and neutral == False:#increase
       setpoint1=setpoint1+1
       print setpoint1
    elif(msg.key=="o" and neutral == False):#decrease
       setpoint1=setpoint1-1
       print setpoint1
    elif msg.key==' ' :
       quit_count=0
       ...
       e2OCAN_RNDB.DATA[6] = RNDB_DRIVE
       print 'f'
    elif(msg.key=="p"):#plotting
       dummy=np.asarray(car_sp)
       plt.xlabel('Time')
       plt.ylabel('Speed')
       plt.plot(dummy[:,2],dummy[:,0],'r',dummy[:,2],dummy[:,1],'b')
       plt.show()
...
pub2 = rospy.Publisher('/setpoint' , Float64,queue_size=10)
